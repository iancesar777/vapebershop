 ...
 function updateImg() {
        if(currentImg > imageCount.length){
         ...
         currentImg = 6;
            ...
            const changetosmallerdevice = window.matchMedia("(max-width:350px)");            
            changetoTablet.addListener(TabletMode);
            changetoMobile.addListener(MobileMode);
            changetosmallerdevice.addListener(SmallerMode)
        ...
        //  change the image slider to mobile size
         ...
         function SmallerMode(screeenwidth) {
            if(screeenwidth.matches) {
                nextImage.style.transform =  `translateX(-${(currentImg - 1) * 250}px)`;
            }
         }

           TabletMode(changetoTablet);
           MobileMode(changetoMobile);
             SmallerMode(changetosmallerdevice);          
